...
type Vector<'t when 't: equality and 't:> ValueType and 't : struct and 't: (new: unit -> 't) and 't :> IEquatable<'t>>
    ...
    interface IHtmlVisualization with
        member x.Draw(attrs:_)  =
            ...
            | 2, rt -> HtmlVisualization.draw_vec2 attrs e.[0].Raw e.[1].Raw |> draw_board
...
module Vector =
    ...
    let vnorm (l:IVector<real>) = RealNumbers.sqrt (vdot l l)
    ...
    let vecset (v:Vector<_> seq) = finite_seq v
    
    let is_orthg (v:Vector<_> seq) =
        seq {for i in 0 .. Seq.length v - 1 do seq { for j in 0 ..Seq.length v - 1 do if i <> j then yield (Seq.item i v) * (Seq.item j v) }} 
        |> Seq.concat
        |> Seq.forall ((=) zero)

    let is_orthn (v:Vector<_> seq) = is_orthg v && v |> Seq.forall (vnorm >> ((=) one))
        
    let gramschimdt_orthn (v:Vector<_> seq) =
        let c (vk:Vector<_>) (uk:Vector<_>)  = (vdot vk uk) * uk in 
        seq { for i in 0 .. Seq.length v - 1 -> 
                let mutable us: Vector<_> list = []   
                let x = Seq.item i v 
                let w = if List.length us > 0 then x - (us |> List.map (c x) |> List.reduce (+)) else x 
                let u = w / vnorm w
                us <- us @ [u]
                u
            } 