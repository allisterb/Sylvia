...
type RealFunction(n:int, f, s, ?vars: ScalarVar<real> list, ?symbol:string) = 
    ...
    interface IRealFunction<RealFunction> with
       ...
       member x.Draw(attrs:_) = 
           ...
           HtmlVisualization.draw_realfun2 attrs ((x :> IRealFunction<RealFunction>).Html()) (x.JSDrawFunc()) |> draw_board
...
type RealFunctionGroupVisualization(_grp:seq<IRealFunction<RealFunction>>) =
    interface IHtmlVisualization with
           member x.Draw(attrs:_) = 
              ...
              HtmlVisualization.draw_realfuns attrs (_grp |> Seq.map(fun x->(x :> IRealFunction<_>).Html()) |> Seq.toArray) (_grp |> Seq.map(fun x ->x.Term.JSDrawFunc()) |> Seq.toArray) |> draw_board
