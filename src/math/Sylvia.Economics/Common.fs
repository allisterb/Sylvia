...
type DemandFunction(f:RealFunction) =
    ...
    interface IHtmlVisualization with
        member x.Draw(attrs:_) =             
            ...
            let e = RealAnalysisOps.Ops.SolveForPosVars x.ScalarVars[0].Expr [eq.Expr]
            let fe = recombine_func_as<real->real> [xxqxx.Var] e.Head
            HtmlVisualization.draw_realfun2 attrs ((x :> IRealFunction<DemandFunction>).Html()) fe |> draw_board
...
type EconomicModel() = 
    ...
    member x.CreateFun(name:string, v:realvar list) = x.SetFun<RealFunction>(name, realfun_s name v)
    member x.CreateProdFun(name:string, v:realvar list) = x.SetFun<ProductionFunction>(name, realfun_s name v |> ProductionFunction)
    member x.CreateUtilFun(name:string, v:realvar list) = x.SetFun<UtilityFunction>(name, realfun_s name v |> UtilityFunction)
    member x.CreateUtilFun(name:string, v:string list) = x.SetFun<UtilityFunction>(name, realfun_s name (v |> List.map x.GetVar) |> UtilityFunction)
...
module Economics =
    ...
    let average (func:RealFunction) = 
     match func.Symbol with
     | None -> RealFunction((fexpr func) / (func.ScalarVars[0])) |> with_attr_tag "Average"
     | Some s -> diff (func.ScalarVars[0]) func |> with_attr_tag "Average" |> with_symbol ("A" + s)

    let demandfun s (func:Scalar<real>) = RealFunction(func, symbol=s) |> DemandFunction
