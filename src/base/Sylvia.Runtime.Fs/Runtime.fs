...
module FsRuntime =
    ...
    let success = 
        function
        | Ok value -> value
        | Error failure -> failwith "This operation failed."

    let init (r: 'T when 'T :> Runtime) = if r.Initialized then r else failwith "This runtime object is not initialized."

    let init' (r: 'T when 'T :> Runtime) = if r.Initialized then Ok r else Error (exn "This runtime object is not initialized.")

    let switch' res f = 
        match res with
        | Ok _ -> tryCatch' f
        | Error failure -> fun _ -> Error failure
    
    let switchAsync' res f =
        match res with
        | Ok _ -> tryCatchAsync' f
        | Error failure -> fun _ -> Error failure
    ...
    let (>>|>) f1 f2  = Result.bind (fun() -> f2) f1

    let (>>>|) r f = r |> init' |> switchAsync' <| f

    let (>>=) f1 f2  = Result.bind f2 f1 
