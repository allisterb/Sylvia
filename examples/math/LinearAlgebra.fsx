...
mcharpoly l A
let parseEigenVectList (infix: string) =
    let e = infix.Split("]],[[[")
    let e0 = e.[0].Remove(0,1) + "]]"
    let e1 = e.[1].Remove((e.[1].Length - 4), 4).Split("]],[[") //|> Array.map(fun i -> i.Split(',')) |> Array.map (Array.toList) |> Array.toList
    e0, e1

parseEigenVectList "[[[1,2],[1,1]],[[[1,2/5]],[[1,1/3]]]]"

[vec [0;0;1]; vec [0;1;0]; vec [2;0;0]] |> is_orthn
...
let V = [vec [1;1;1;1]; vec [-1;4;4;-1]; vec [4;-2;2;0]]

(gramschimdt_orthn V) |> Seq.item 2
(*
